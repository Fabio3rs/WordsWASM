...
async function run(format, args, input, db = database) {
  return new Promise((resolve) => {
    const child = spawn(cli, ["--database", db, "--format", format, ...args]);
    let stdout = "";
    let stderr = "";
    let error;
...
async function documents(format, args, input, db) {
  const result = await run(format, args, input, db);
...
for (const format of ["analysis-v3", "search-v3"]) {
  ...
  const plain = await run(format, baseArgs, text);
  assert.equal(plain.status, 0, `${plain.error ?? plain.stderr}`);
  assert.equal((await run(format, [...baseArgs, "--filter-trim=none"], text)).stdout, plain.stdout);
  const rawDocs = plain.stdout.trim().split("\n").map(JSON.parse);
  const filteredDocs = await documents(format, [...baseArgs, "--filter-trim", reasons.join(",")], text);
  ...
  for (const reason of reasons) {
    const selected = await documents(format, [...baseArgs, `--filter-trim=${reason}`], text);
    for (let index = 0; index < forms.length; ++index) {
  ...
  const pretty = await run(format, ["--pretty", `--filter-trim=${reasons.join(",")}`, "reg amo"]);
...
for (const args of [["--filter-trim=bad"], ["--filter-trim="], ["--filter-trim"],
  ...
  assert.equal((await run("analysis-v3", args)).status, 2);
}
for (const format of ["analysis", "search", "analysis-v2", "search-v2"]) {
  assert.equal((await run(format, ["--filter-trim=deponent-active-form", "res"])).status, 2);
  assert.equal((await run(format, ["--filter-trim=none", "res"])).status, 0);
}
assert.deepEqual(
  await documents("analysis-v3", ["--filter-trim=deponent-active-form,deponent-active-form", "res"]),
  await documents("analysis-v3", ["--filter-trim=deponent-active-form", "res"]),
);
  ...
  for (const db of [database, ...(searchDatabase ? [searchDatabase] : [])]) {
    ...
    try {
      for (const form of forms) {
        ...
        const [native] = await documents("search-v3", ["--two-words=legacy", `--filter-trim=${reasons.join(",")}`, "--input", "-"], form, db);
